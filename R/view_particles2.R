...
routes = dbGetQuery(con, "SELECT routes.route_id, routes.route_short_name as number, MAX(shapes.dist_traveled) AS len FROM routes, shapes WHERE routes.shape_id=shapes.shape_id AND route_id LIKE '%v54.27' GROUP BY shapes.shape_id")
...
## shapes = dbGetQuery(con, "SELECT * FROM shapes WHERE shape_id IN(SELECT DISTINCT shape_id FROM routes WHERE route_short_name IN ('274','277','258','NEX','881')) ORDER BY shape_id, seq")
shapes = dbGetQuery(con, "SELECT * FROM shapes ORDER BY shape_id, seq")

xr = range(shapes$lng)#extendrange(shapes$lng)
yr = range(shapes$lat)#extendrange(shapes$lat)
bbox = c(xr[1], yr[1], xr[2], yr[2])
akl = get_stamenmap(bbox, zoom = 10, maptype = "toner-lite")
ggmap(akl)
...
while (TRUE) {
    ...
    try({
        particles = dbGetQuery(con, sprintf("SELECT vehicle_id, timestamp, (particles.trip_id) as trip_id, route_id, (distance) as distance, log_likelihood, initialized, etas, lat, lng FROM particles, trips WHERE particles.trip_id=trips.trip_id AND timestamp > %s", t0))
    })
    ...
    with(particles, {#[particles$progress > 1 & particles$progress < 99, ], {
        ...
        ##     tdiff <- round(as.numeric(Sys.time()) - (timestamp[vehicle == v])[1])
        ##     text(0, which(levels(vehicle) == v), sprintf("(%ds ago)", tdiff), pos = 1, cex = 0.8)
        ##     invisible(NULL)
        ## })
        ...
        ## dev.set()
        pl <- ggmap(akl) +
            geom_path(aes(lng, lat, group = shape_id), data = shapes, color = "steelblue", lwd = 1) +
